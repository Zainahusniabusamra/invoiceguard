...
def run_case(case_file: str) -> Dict[str, Any]:
    ...
    result = generate_report(case_id, "MATCHED", [], str(case_path), "")

    try:
        payload = json.loads(case_path.read_text(encoding="utf-8"))
    except FileNotFoundError:
        issue = _issue("MISSING_DOCUMENT", f"Input file not found: {case_path}", [str(case_path)])
        result = generate_report(case_id, "ERROR", [issue], str(case_path), "")
        _save_result(result, case_id)
        return result
    except json.JSONDecodeError as exc:
        issue = _issue("MISSING_DOCUMENT", f"Malformed JSON in {case_path}: {exc.msg} at line {exc.lineno} column {exc.colno}.", [str(case_path)])
        result = generate_report(case_id, "ERROR", [issue], str(case_path), "")
        _save_result(result, case_id)
        return result
    ...
    gr_items = gr.get("received_items") if isinstance(gr.get("received_items"), list) else []
    ...
    for idx, item in enumerate(gr_items):
        if not isinstance(item, dict):
    ...
    for idx, item in enumerate(inv_items):
        ...
        gr_qty = _to_float(gr_by_sku.get(sku_key, {}).get("item", {}).get("quantity"))
        ...
        if inv_qty > (gr_qty if gr_qty is not None else -1):
            has_qty_issue = True
    ...
    result = generate_report(case_id, status, issues, str(case_path), "")
    final_result = result
